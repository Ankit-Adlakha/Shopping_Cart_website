...
const Product = ({post}) => {
  ...
  return (
    <div className='flex flex-col items-center justify-between border-2 border-[#00095] shadow-lg hover:scale-110 hover:shadow-2xl transition duration-300 ease-in gap-3 p-4 mt-10 ml-5 rounded-xl'>
        <div>
            <p className='text-gray-700 font-semibold text-lg text-left truncate w-40 mt-1'>{post.title.substring(0,16)+"..."}</p>
        </div>
        <div>
            <p className='w-40 text-gray-400 font-normal text-[10px] text-left'>{post.description.split(" ").slice(0,10).join(" ")+"..."}</p>
        </div>
        <div className='h-[180px]'>
            <img src={post.image} alt='Icon' className='h-full w-full'/>
        </div>
